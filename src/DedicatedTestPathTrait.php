...
trait DedicatedTestPathTrait
{
     ...
     * @internal
     */
    private static string $testBaseFolder = 'var/tests';
    ...
    /**
     * Set to true to remove the test folder at teardown.
     */
    ...
    protected function getTestPath(?string $subFolder = null): string
    {
        if ($this->testPath === null) {
            $this->testPath = $this->getAbsoluteTestBasePath() . '/' . $this->getTestDirectory(
    ...
    {
        if ($this->cleanupTestPath && self::getFilesystem()->exists($this->getTestPath())) {
    ...
    private function getTestDirectory(string $dir, string $prefix): string
    {
        ...
        for ($i = 0; $i < 10; ++$i) {
            ...
            if ($filesystem->exists($dir . '/' . $testDir)) {
                continue;
            }
    ...
    {
        return self::getProjectRootPath() . '/' . self::getTestBaseFolder();
    }
}
